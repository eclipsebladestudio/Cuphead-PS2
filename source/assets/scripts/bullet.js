const HALF_SCREEN_WIDTH = 640
const HALF_SCREEN_HEIGHT = 224

const WEAPON_SHOOT_FRAMES_PATH = "/source/assets/player/weapon_shoot/";
...
export class Bullet {
    ...
    update(playerX, playerY, entitys) {
        ...
        if (
            this.x < playerX - HALF_SCREEN_WIDTH || this.x > playerX + HALF_SCREEN_WIDTH ||
            this.y < playerY - HALF_SCREEN_HEIGHT || this.y > playerY + HALF_SCREEN_HEIGHT
        ) {
            this.active = false;
            return;
        }
        ...
        shootAnimation[this.angle][this.frame].draw(
            shootAnimation[this.angle][this.frame].width > 0
                ? this.x - this.w
                : (this.x + this.w) - shootAnimation[this.angle][this.frame].width / 2,
            this.y - 10
        );
    }
